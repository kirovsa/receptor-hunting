...
An R port of `extract_primate_receptors.py` with identical logic and output
format.
...
1. **UniProt query** – fetches Swiss-Prot (reviewed) human proteins annotated
   as a *receptor* (keyword `KW-0675`) with subcellular location *Cell
   membrane*.
2. **Ensembl orthology** – queries the [Ensembl REST API][ensembl-rest]
   homology endpoint for each receptor that has an Ensembl cross-reference.
3. **Primate filter** – consults the [NCBI Taxonomy API][ncbi-tax] for each
   orthologue species (results cached in-process); keeps only receptors whose
   *every* orthologue belongs to order Primates (taxon ID 9443).
4. **Output** – results are written to a TSV file and a summary table is
   printed to stdout.
...
```bash
# Write results to the default file (primate_membrane_receptors.tsv)
Rscript extract_primate_receptors.R

# Specify a custom output path
Rscript extract_primate_receptors.R results/my_output.tsv

# Enable verbose debug logging
Rscript extract_primate_receptors.R --debug
```
